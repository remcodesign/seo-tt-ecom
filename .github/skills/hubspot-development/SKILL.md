...
description: 'Use when building or maintaining HubSpot projects in this Laravel repository, including CRM UI extensions, app cards, actions, agent tools, webhooks, request signing, Laravel quote workflows, and HubSpot CLI development.'
argument-hint: 'Describe the HubSpot component, integration, endpoint, or CLI workflow.'
---
...
### Generated card project package layout

- Projects created with `hs project create --features card` keep the card's `package.json`, `tsconfig.json`, ESLint configuration, and source files together under `src/app/cards/`.
- Do not assume the HubSpot project has a root `package.json`. Run npm commands from the directory that owns the package file:
...
- Keep card dependencies in the HubSpot project, not in the Laravel application's root `package.json`.
...
### Local development account troubleshooting

- `hs project validate` validates the local project metadata independently of account access. Run it before investigating `hs project dev` failures.
- `hs account info` and `hs account list` show the active account, account type, and configured test accounts. Do not expose the contents of `~/.hscli/config.yml` because it contains local authentication configuration.
- `hs project dev` may perform a sandbox entitlement check before starting the local server. A log such as:

  ```text
  GET sandbox-hubs/v1/parent/<account-id>/usage
  403 MISSING_SCOPES
  The scope needed for this API call isn't available for public use.
  ```

  indicates an account capability or account-type problem, not invalid card metadata. In particular, a standard account may not have access to the development-sandbox usage API. The app's `requiredScopes` and the CLI personal access key scopes are separate concerns.
- When a developer/project account has a HubSpot test account, run local development with both accounts explicitly:

  ```shell
  hs project dev \
    --project-account <developer-account-id> \
    --testing-account <test-account-id> \
    --debug
  ```

- The CLI requires `--project-account` and `--testing-account` together. Use a development sandbox or another account type explicitly supported by the current HubSpot plan when the test-account workflow is unavailable.
