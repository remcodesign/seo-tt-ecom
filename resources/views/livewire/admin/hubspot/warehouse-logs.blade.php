<div class="space-y-6">
    <div class="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
        <div>
            ...
            <p class="mt-2 max-w-3xl text-sm leading-6 text-slate-600">
                Browse durable workflow tasks, compare outcomes, and inspect the redacted execution trace saved for each
                run.
            </p>
        </div>
        ...
        <div class="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
            <div>
                ...
                <p class="mt-2 text-sm leading-6 text-slate-600">
                    Search IDs and outcomes across the durable task history.
                </p>
            </div>
            <button
                type="button"
    ...
    <div class="grid items-start gap-6 lg:grid-cols-[minmax(0,1fr)_minmax(0,2fr)]">
        <section class="overflow-hidden rounded-3xl bg-white shadow-sm ring-1 ring-slate-200">
            ...
            <div class="divide-y divide-slate-100">
                @forelse ($tasks as $task)
                    ...
                    @endphp
                    <div
                        wire:key="warehouse-task-{{ $task->id }}"
                        class="grid grid-cols-[minmax(0,1fr)_auto] gap-4 px-6 py-3.5 {{ $selectedTaskId === $task->id ? 'bg-slate-50' : '' }}"
                    >
                        <div class="min-w-0 space-y-2 text-xs">
                            <div class="flex min-w-0 flex-wrap items-center gap-x-4 gap-y-1">
                                ...
                                <span
                                    class="truncate font-mono font-semibold text-slate-900"
                                    title="{{ $task->deal_id }}"
                                >Deal {{ $task->deal_id }}</span>
                            </div>
                            <div class="flex min-w-0 flex-wrap items-center gap-x-4 gap-y-1 text-slate-500">
                                ...
                                <span class="max-w-64 truncate font-mono" title="{{ $task->callback_id }}"
                                    >Callback {{ $task->callback_id }}</span>
                            </div>
                        </div>
                        <button
                            type="button"
                            wire:click="selectTask('{{ $task->id }}')"
                            class="cursor-pointer self-center rounded-xl border border-slate-300 bg-white px-3 py-2 text-xs font-semibold text-slate-700 transition hover:border-slate-500 hover:bg-slate-50"
                        >
                ...
                @empty
                    <div class="px-6 py-12 text-center text-sm text-slate-500">
                        No warehouse workflow tasks match these filters.
                    </div>
                @endforelse
            </div>
        ...
        <section class="rounded-3xl bg-white p-6 shadow-sm ring-1 ring-slate-200 sm:p-8 lg:sticky lg:top-6">
            <div class="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
                <div>
                    <p class="text-xs font-semibold tracking-[0.18em] text-slate-500 uppercase">Selected task</p>
                    @if ($selectedTask)
                        <h2 class="mt-2 font-mono text-xl font-semibold break-all text-slate-900">
                            {{ $selectedTask->id }}
                        </h2>
                        <p class="mt-2 text-sm text-slate-600">
                            Full persisted task details and redacted execution trace.
                        </p>
                    @else
            ...
            @if ($selectedTask)
                <dl class="mt-6 grid gap-x-4 gap-y-3 text-sm sm:grid-cols-2">
                    @foreach ([
                        ...
                        <div>
                            ...
                            <dd class="mt-1 font-mono text-xs font-semibold break-all text-slate-900">{{ $value }}</dd>
                        </div>
                    @endforeach
                </dl>
                ...
                @if (is_array($selectedTask->result))
                    <div class="mt-8">
                        ...
                        <p class="mt-2 text-sm text-slate-600">
                            {{ $selectedTask->result['summary'] ?? 'Recommendation result' }}
                        </p>
                    ...
                    <div class="mt-8">
                        ...
                        <div class="mt-4 space-y-3">
                            @foreach ($selectedTask->debug_trace as $trace)
                                <details
                                    wire:key="warehouse-trace-{{ $selectedTask->id }}-{{ $loop->index }}"
                                    class="rounded-2xl border border-slate-200 bg-slate-50 p-4"
                                    @if ($loop->last) open @endif
                                >
                ...
                @else
                    <div class="mt-8 rounded-2xl bg-slate-50 p-5 text-sm text-slate-600">
                        No debug trace has been recorded for this task.
                    </div>
                @endif
            @else
                <div class="mt-6 rounded-2xl border border-dashed border-slate-300 bg-slate-50 p-5 text-sm leading-6 text-slate-600">
                    Choose a task from the list to keep its metadata, result, and debug trace visible here while you
                    browse.
                </div>
            @endif
        </section>
