...
<body class="min-h-screen bg-slate-50 text-slate-900 antialiased">
    <div class="min-h-screen">
        <header class="border-b border-slate-200 bg-white/80 backdrop-blur-sm">
            <div class="max-w-8xl mx-auto flex items-center justify-between gap-4 px-4 py-4 sm:px-6 lg:px-8">
                <div>
                    <a href="{{ route('admin.dashboard') }}" class="text-lg font-semibold text-slate-900"
                        >{{ config('app.name', 'Laravel') }} Admin</a>
        ...
        <main class="max-w-8xl mx-auto px-4 py-10 sm:px-6 lg:px-8">{{ $slot }}</main>
    </div>
    @livewireScripts
</body>
</html>
